<x-layouts.shop title="Home">
    {{-- Hero Section --}}
    <section class="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500">
        <div class="absolute inset-0 bg-black/20"></div>
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32">
            <div class="text-center">
                <h1 class="text-4xl md:text-6xl font-extrabold text-white mb-6 leading-tight">
                    Find Your Perfect
                    <span class="block text-yellow-300">Uma Musume</span>
                </h1>
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="{{ route('products.index') }}" class="inline-flex items-center justify-center px-8 py-3 bg-white text-indigo-700 font-semibold rounded-xl hover:bg-gray-100 transition shadow-lg text-lg">
                        Browse Horses
    ...
    <section class="bg-white dark:bg-gray-800 border-y border-gray-200 dark:border-gray-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="flex items-center justify-between mb-10">
                ...
                <a href="{{ route('products.index') }}" class="hidden sm:inline-flex items-center text-indigo-600 dark:text-indigo-400 font-medium hover:text-indigo-700 transition">
                    View All
                ...
                @foreach($newest as $listing)
                    <a href="{{ route('products.show', $listing) }}" class="group flex bg-gray-50 dark:bg-gray-700/50 rounded-xl overflow-hidden hover:shadow-md transition-all duration-300 border border-gray-100 dark:border-gray-600">
                        ...
                        <div class="p-4 flex flex-col justify-center">
                            <span class="text-xs text-indigo-600 dark:text-indigo-400 font-medium">{{ $listing->horse->breed?->breed_name ?? 'Unknown' }}</span>
                            ...
                            <span class="text-lg font-bold text-indigo-600 dark:text-indigo-400 mt-1">¥{{ number_format($listing->list_price) }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
    ...
    {{-- CTA Section --}}
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-3xl p-8 md:p-12 text-center">
            ...
            @guest
                <a href="{{ route('register') }}" class="inline-flex items-center px-8 py-3 bg-white text-indigo-700 font-semibold rounded-xl hover:bg-gray-100 transition shadow-lg">
                    Get Started
                </a>
            @else
                <a href="{{ route('products.index') }}" class="inline-flex items-center px-8 py-3 bg-white text-indigo-700 font-semibold rounded-xl hover:bg-gray-100 transition shadow-lg">
                    Browse Horses
                </a>
            @endguest
        </div>
